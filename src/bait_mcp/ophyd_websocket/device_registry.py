...
class DeviceRegistry:
    ...
    def _load_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            module = self._import_module_for(file_path)
            ...
            # BITS/Guarneri layout: devices live inside an ``oregistry`` (or any
            # ophyd-registry-like attribute), not bound at module top level.
            for name, obj in vars(module).items():
                if name.startswith("_"):
                    continue
                names = self._device_names_of(obj)
                if names is None:
                    continue
                for dev_name in names:
                    if dev_name in self._devices:
                        continue
                    self.add_device(dev_name, obj[dev_name])
                    devices_found += 1
    ...
    def _import_module_for(self, file_path: Path):
        """Import file_path. If it lives inside a Python package, import as
        ``pkg.subpkg.stem`` so relative imports inside the file resolve.
        Falls back to ``spec_from_file_location`` for true single-file scripts.
        """
        parent = file_path.parent
        if (parent / "__init__.py").exists():
            pkg_parts = [parent.name]
            ancestor = parent.parent
            while (ancestor / "__init__.py").exists():
                pkg_parts.insert(0, ancestor.name)
                ancestor = ancestor.parent
            module_name = ".".join(pkg_parts + [file_path.stem])
            sys.path.insert(0, str(ancestor))
            try:
                return importlib.import_module(module_name)
            finally:
                if str(ancestor) in sys.path:
                    sys.path.remove(str(ancestor))
    ...
    @staticmethod
    def _device_names_of(obj: Any) -> Optional[List[str]]:
        """If obj duck-types as an ophyd-registry-style mapping (has
        ``device_names`` and is subscriptable), return its names as a list.
        Otherwise None. Duck-typed to avoid a hard ophyd-registry dep and to
        accept Guarneri's ``Instrument.devices``.
        """
        names = getattr(obj, "device_names", None)
        if names is None or not callable(getattr(obj, "__getitem__", None)):
            return None
        try:
